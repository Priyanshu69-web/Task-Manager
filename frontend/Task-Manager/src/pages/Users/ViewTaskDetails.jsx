...
const ViewTaskDetails = () => {
    ...
    const { triggerRefreshDashboard } = useContext(TaskContext);
    ...
    const updateTodochecklist = async (index) => {
        ...
        // Update state locally immediately for UI responsiveness
        setTask({ ...task, todoChecklist });

        try {
            const response = await axiosInstance.put(API_PATHS.TASKS.UPDATE_TODO_CHECKLIST(task._id), { todoChecklist });
            console.log("Update todoChecklist response:", response);
            if (response.status === 200) {
                // Fetch updated task details to get latest status
                const taskResponse = await axiosInstance.get(API_PATHS.TASKS.GET_TASK_BY_ID(task._id));
